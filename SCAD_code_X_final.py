...
def compute_phi_lambda(X, lambda_val):
    ...
    Q_lambda = 0
    for sigma in S:
        if abs(sigma) <= lambda_val:
            Q_lambda += lambda_val * abs(sigma)
        elif abs(sigma) <= b * lambda_val:
            Q_lambda += (2 * b * lambda_val * abs(sigma) - sigma**2 - lambda_val**2) / (2 * (b - 1))
        else:
            Q_lambda += (b + 1) * lambda_val**2 / 2
...
def compute_grad_L_n_lambda_tilde(X, lambda_val):
    ...
    for i, sigma in enumerate(S):
        if abs(sigma) <= lambda_val:
            ...
            grad_Q_lambda += ((2 * b * lambda_val * np.sign(sigma) - 2 * sigma) / (2 * (b - 1))) * np.outer(U[:, i], Vt[i, :])
        else:
            grad_Q_lambda += 0
